...
!use cvmix_utils,              only : cvmix_update_tke
...
subroutine init_tke(c_k, c_eps, cd, alpha_tke, mxl_min, KappaM_min, KappaM_max, &
                    tke_mxl_choice, do_ubound_dirichlet, do_lbound_dirichlet, &
...
if (present(c_k)) then
  call cvmix_put_tke('c_k', c_k, tke_userdef_constants)
else
  call cvmix_put_tke('c_k',0.1d0 , tke_userdef_constants)
end if

if (present(c_eps)) then
  call cvmix_put_tke('c_eps', c_eps, tke_userdef_constants)
else
  call cvmix_put_tke('c_eps', 0.7d0, tke_userdef_constants)
end if

if (present(cd)) then
  call cvmix_put_tke('cd', cd, tke_userdef_constants)
else
  call cvmix_put_tke('cd', 3.75d0, tke_userdef_constants)
end if

if (present(alpha_tke)) then
  call cvmix_put_tke('alpha_tke', alpha_tke, tke_userdef_constants)
else
  call cvmix_put_tke('alpha_tke', 30.d0, tke_userdef_constants)
end if

if (present(mxl_min)) then
  call cvmix_put_tke('mxl_min', mxl_min, tke_userdef_constants)
else
  call cvmix_put_tke('mxl_min', 1.d-8, tke_userdef_constants)
end if

if (present(KappaM_min)) then
  call cvmix_put_tke('kappaM_min', KappaM_min, tke_userdef_constants)
else
  call cvmix_put_tke('kappaM_min', 0.d0, tke_userdef_constants)
end if

if (present(KappaM_max)) then
  call cvmix_put_tke('kappaM_max', KappaM_max, tke_userdef_constants)
else
  call cvmix_put_tke('kappaM_max', 100.d0, tke_userdef_constants)
end if

if (present(tke_mxl_choice)) then
  call cvmix_put_tke('tke_mxl_choice', tke_mxl_choice, tke_userdef_constants)
else
  call cvmix_put_tke('tke_mxl_choice', 2, tke_userdef_constants)
end if
...
!    CALL finish(method_name,'ERROR:handle_old_vals can only be 1 to 3')
  ...
  call cvmix_put_tke('handle_old_vals', handle_old_vals, tke_userdef_constants)
else
  call cvmix_put_tke('handle_old_vals', 1, tke_userdef_constants)
end if

if (present(tke_min)) then
  call cvmix_put_tke('tke_min', tke_min, tke_userdef_constants)
else
  call cvmix_put_tke('tke_min', 1.d-6, tke_userdef_constants)
end if

if (present(tke_surf_min)) then
  call cvmix_put_tke('tke_surf_min', tke_surf_min, tke_userdef_constants)
else
  call cvmix_put_tke('tke_surf_min', 1.d-4, tke_userdef_constants)
end if

if (present(do_ubound_dirichlet)) then
  call cvmix_put_tke('do_ubound_dirichlet', do_ubound_dirichlet,            &
                     tke_userdef_constants)
else
  call cvmix_put_tke('do_ubound_dirichlet', .false., tke_userdef_constants)
end if

if (present(do_lbound_dirichlet)) then
  call cvmix_put_tke('do_lbound_dirichlet', do_lbound_dirichlet,            &
                     tke_userdef_constants)
else
  call cvmix_put_tke('do_lbound_dirichlet', .false., tke_userdef_constants)
end if

if (present(only_tke)) then
  call cvmix_put_tke('only_tke', only_tke, tke_userdef_constants)
else
  call cvmix_put_tke('only_tke', .true., tke_userdef_constants)
end if
...
! This subroutine is necessary to handle old/new values and to hand over the 
! TKE parameters set in previous subroutine
...
! end in/out
  ...
  logical :: only_tke, do_ubound_dirichlet, do_lbound_dirichlet
  ...
  do_ubound_dirichlet = tke_constants_in%do_ubound_dirichlet
  do_lbound_dirichlet = tke_constants_in%do_lbound_dirichlet
  ...
  !tke_surf_min = 1.d-4
  !only_tke = .true.
  !do_ubound_dirichlet = .false.
  !do_lbound_dirichlet = .false.
...
!!!    write(*,*) 'do_ubound_dirichlet = ', do_ubound_dirichlet
!!!    write(*,*) 'do_lbound_dirichlet = ', do_lbound_dirichlet
...
!OUT   
  ...
  select case(trim(varname))
    ...
    case('do_ubound_dirichlet')
      tke_constants_out%do_ubound_dirichlet=val  
    case('do_lbound_dirichlet')
      tke_constants_out%do_lbound_dirichlet=val  
    case DEFAULT
